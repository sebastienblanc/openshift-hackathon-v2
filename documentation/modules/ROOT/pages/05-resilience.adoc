...
**Notes** : nous avons utilisé ici la fonctionnalité "HorizontalPodAutoscaler" qui permet d'ajuster notre nombre de PODs dynamiquement.  
Il existe également le "VerticalPodAutoscaler" (https://docs.openshift.com/container-platform/4.8/nodes/pods/nodes-pods-vertical-autoscaler.html) qui permet de modifier dynamiquement les ressources affectées à des PODs.  
